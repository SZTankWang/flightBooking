...
def viewBookingAgent():
    ...
    month_ticket_result = [{column: float(value) if type(value) == decimal.Decimal else value for column, value in rowproxy.items()} for rowproxy in resultproxy1][0]
    resultproxy3 = db.session.execute(text("CALL eflight.get_top_ticket_agent(:p1,365)"),{"p1":staffID})
    year_ticket_result = [{column: float(value) if type(value) == decimal.Decimal else value for column, value in rowproxy.items()} for rowproxy in resultproxy1][0]
